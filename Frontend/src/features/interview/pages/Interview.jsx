...
const Interview = () => {
    ...
    const handleResumeDownload = async () => {
        if (!interviewId || resumeDownloadLoading) {
            return
        }
        ...
        try {
            ...
            console.error(error)
            setResumeDownloadError(error?.message || 'Failed to download resume.')
        } finally {
    ...
    const scoreSummary =
        report.matchScore >= 80 ? 'Excellent role alignment' :
            report.matchScore >= 60 ? 'Good potential with focused prep' :
                'Needs stronger positioning for this role'
            ...
            <div className='interview-layout'>
                <nav className='interview-nav'>
                    <div className='nav-content'>
                        {user?.username && (
                            <p className='interview-nav__user'>@{user.username}</p>
                        )}
                        <button
                            className='button interview-nav__home-btn'
                            onClick={() => navigate('/')}
                        >
                            Home
                        </button>
                        <p className='interview-nav__label'>Sections</p>
                        <p className='interview-nav__meta'>{report.title || 'Interview Plan'}</p>
                        {NAV_ITEMS.map(item => (
                            <button
                                key={item.id}
                    ...
                    >
                        ...
                        {resumeDownloadLoading ? 'Downloading Resume...' : 'Download Resume'}
                    </button>
                    {resumeDownloadError && (
                        <p className='interview-nav__error' aria-live='polite'>
                            {resumeDownloadError}
                        </p>
                    )}
                ...
                <aside className='interview-sidebar'>
                    ...
                    <div className='skill-gaps'>
                        ...
                        <div className='skill-gaps__list'>
                            {skillGaps.length === 0 && (
                                <span className='skill-tag skill-tag--low'>No major gaps detected</span>
                            )}
                            {skillGaps.map((gap, i) => (
